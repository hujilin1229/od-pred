...
flags.DEFINE_string('config_filename', './conf/base_fc_config.json', 'Configuration filename for restoring the model.')
...
flags.DEFINE_integer('hopk', 2, 'Hopk to construct the adjacent matrix')
...
def main(_):
    ...
    with open(FLAGS.config_filename) as f:
        ...
        for name in ['batch_size', 'cl_decay_steps', 'epochs', 'horizon', 'learning_rate', 'l1_decay',
                     ...
                     'zone', 'scaler', 'data_format', 'num_gpus', 'mode', 'drop_out', 'shuffle_training',
                     'optimizer', 'fc_method', 'trace_ratio', 'hopk']:
        ...
        with tf.Session(config=tf_config) as sess:
            supervisor = FCRNNSupervisor(traffic_reading_df=dataset_f, adj_mx=adj_mx,
