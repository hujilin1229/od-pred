...
from lib import coarsening, tf_utils, metrics, metrics_weight
from model.fcrnn_model import FCRNNModel
from model.tf_model_supervisor import TFModelSupervisor


class FCRNNSupervisor(TFModelSupervisor):
    """
    ...
    def __init__(self, traffic_reading_df, adj_mx, config,
        ...
        self._fc_method = config['fc_method']
        self._hopk = config['hopk']
        if not os.path.exists('./result/'):
            os.makedirs('./result/')
        self._result_dir = './result/'

        super(FCRNNSupervisor, self).__init__(config, df_data=traffic_reading_df)
    ...
    def _prepare_train_val_test_data(self):
        ...
        coarsening_levels = self._get_config('coarsening_levels')

        test_batch_size = self._get_config('test_batch_size')
        add_time_in_day = self._get_config('add_time_in_day')

        coarsened = self._get_config('coarsen')
        if coarsened:
            self._graphs, perm = coarsening.coarsen(
                self._adj_mx, levels=coarsening_levels, self_connections=True)
        else:
            perm = None
            self._graphs = [self._adj_mx] * coarsening_levels
    ...
    def _build_train_val_test_models(self):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('MGRNN', reuse=False):
                train_model = FCRNNModel(is_training=True, config=train_config, scaler=self._scaler,
                                         adj_mxs=self._graphs, adj_origin=self._adj_mx)
        with tf.name_scope('Val'):
            with tf.variable_scope('MGRNN', reuse=True):
                val_model = FCRNNModel(is_training=False, config=test_config, scaler=self._scaler,
                                       adj_mxs=self._graphs, adj_origin=self._adj_mx)
        with tf.name_scope('Test'):
            with tf.variable_scope('MGRNN', reuse=True):
                test_model = FCRNNModel(is_training=False, config=test_config, scaler=self._scaler,
                                        adj_mxs=self._graphs, adj_origin=self._adj_mx)
    ...
    def _train(self, sess, **kwargs):
        ...
        while self._epoch <= epochs:
            ...
            train_results = FCRNNModel.run_epoch(sess, self._train_model,
            ...
            val_results = FCRNNModel.run_epoch(sess, self._val_model, inputs=self._x_val,
    ...
    def _test_and_write_result(self, sess, global_step, **kwargs):
        ...
        start_time = time.time()
        test_results = FCRNNModel.run_epoch(sess, self._test_model, inputs=self._x_test,
        ...
        if mode == 'avg':
            ...
            for i in range(pred_shape[1]):
                ...
                message = 'Horizon %d, kl:%.4f, jsd:%.4f, emd:%.4f, l2:%.4f, %.6f s' % (
                    i + 1, kl, jsd, emd, l2,
                    (end_time - start_time)/pred_shape[0])
                self._logger.info(message)
    ...
    def _write_results_into_df(self, y_preds, dist_mx):
        ...
        dest_df = os.path.join(
            self._result_dir, '{2}_{0}_S{1}_s6h3_S3H3.csv'.format(self._zone, self._sample_rate, self._fc_method))
    ...
    def _generate_run_id(config):
        ...
        fc_method = config.get('fc_method')
        structure = '-'.join(
        ...
        if filter_type == 'random_walk':
            ...
            filter_type_abbr = 'DR'
        run_id = 'fcrnn_{}_h_{}_{}_lr_{}_bs_{}_d_{}_sl_{}_{}_{}_{}_{}_{}_{}_{}/'.format(
            filter_type_abbr, horizon, structure, learning_rate, batch_size,
            dropout, seq_len, loss_func, zone, data_format, sample_rate, time_id,
            fc_method, config.get('trace_ratio'))
        return run_id
