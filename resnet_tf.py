...
def load_data(server_name, borough, start_date, end_date,
    ...
    if os.path.exists(train_file) and os.path.exists(val_file) and os.path.exists(test_file) and os.path.exists(
        ...
        df_data = df_data[(df_data.manhat_dist > 0)]
...
def load_data_cd(server_name, borough, start_date, end_date,
    ...
    if os.path.exists(train_file) and os.path.exists(val_file) and os.path.exists(test_file) and os.path.exists(
        ...
        df_mr_cd = pd.read_csv('./data/{0}/MURA/mr_cd.txt'.format(server_name), sep=';')
...
def loss_func(tt, dist, spatial_error,
              temporal_error, tt_label,
              dist_label, spatial_lambda, temporal_lambda):

    normal_loss = tf.reduce_mean(tf.abs(tt - tt_label)) + \
                      tf.reduce_mean(tf.abs(dist - dist_label))
    print("Normal loss is ", normal_loss)
...
def construct_compare_od_hist():
    ...
    with open('./data/nyc/MURA/edges_hop8_sigma_9.pickle', 'rb') as f:
        edges = pickle.load(f)
    with open('./data/nyc/MURA/edge_dist.pickle', 'rb') as f:
...
def mura_basic(log_dir):
    ...
    model_summary()
    print("optimizing....")
    accuracy = tf.reduce_mean(tf.abs(tt_output - tt))

    print("tt", tt)
    print("dist", dist)
    print("spatial_e", spatial_error)
    print("temporal_e", temporal_error)
    ...
    while num_epochs <= 100:
        ...
        if step % display_step == 0 or step == 1:
            ...
            print("Step " + str(step * batch_size) + ", Minibatch Loss= " + \
                  ...
                  "{:.5f}".format(acc) + ", Validating MAPE {:.5f} ".format(pred_mae))
