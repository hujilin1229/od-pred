...
def hist_figure():
    ...
    # for i, keyf in enumerate(['MR', 'RNN', 'BF', 'AF']):
    #     print(keyf)
    ...
    ylims = [0.5, 0.9, 0.5]
    leg_loc = [(0.1, 0.7), (0.1, 0.7), (0.1, 0.2)]
    for mi, method in enumerate(['EMD', 'KL', 'JS']):
        ...
        barx2 = []
        y_2 = []
        for i, key in enumerate(['MR', 'RNN', 'BF', 'AF']):
            if method == 'JS' and key == 'MR':
                continue
            df_i = df_dict[key]
        ...
        ax2.bar(barx2, y_2, align='edge', width=3, hatch='//', color='g')

        ax2.set_ylim([0.0, 110.0])
        ax2.set_ylabel('Percentage (%)', fontdict=sec_label_font)
        ax2.tick_params('y', colors='g')

        ax.legend(loc=leg_loc[mi], labelspacing=label_space, ncol=2, frameon=False)
    ...
    ylims = [0.4, 0.8, 0.5]
    leg_loc = [(0.05, 0.75), (0.05, 0.75), (0.05, 0.75)]
    for mi, method in enumerate(['EMD', 'KL', 'JS']):
        label = 0
        barx2 = []
        y_2 = []
        ...
        ax2.bar(barx2, y_2, align='edge', width=0.5, hatch='//', color='g')

        ax2.set_ylim([0.0, 135.0])
        ax2.set_ylabel('Percentage (%)', fontdict=sec_label_font)
        ax2.tick_params('y', colors='g')
        ...
        ax.legend(loc=leg_loc[mi], labelspacing=label_space, frameon=False, ncol=2)
        ax.set_ylim([0.0, ylims[mi]])
        ax.set_ylabel(method, fontdict=label_font)
