...
class GBRSupervisor(FCRNNSupervisor):
    ...
    def __init__(self, traffic_reading_df, adj_mx, config,
        ...
        super(GBRSupervisor, self).__init__(traffic_reading_df, adj_mx, config,
                 origin_df_file, nodes)
                ...
                for h_i in range(self._horizon):
                    ...
                    for out_i in range(self._output_dim):
                        clf = GradientBoostingRegressor(loss='quantile', alpha=0.95,
                                                        ...
                                                        min_samples_split=9)
                        clf.fit(train_data_i_j_hi, train_y_i_j_hi[:, out_i])
                        model_out.append(clf)
                    model_hi.append(model_out)
    ...
    def _analyze_result_hist(self, y_preds, labels, weights):
        ...
        self._logger.info('gdbt Results...')
        ...
        if mode == 'avg':
            ...
            for i in range(pred_shape[1]):
                ...
                tf_utils.add_simple_summary(self._writer,
                                            ...
                                            global_step=global_step)
                end_time = time.time()
                message = 'Horizon %d, kl:%.4f, label_kl:%.4f, emd:%.4f, l2:%.4f, %ds' % (
                    i + 1, kl, label_kl, l2, emd, end_time - start_time)
                message = 'Horizon %d, kl:%.4f, jsd:%.4f, emd:%.4f, l2:%.4f' % (
    ...
    def _generate_run_id(config):
        ...
        if filter_type == 'random_walk':
            ...
            filter_type_abbr = 'DR'
        run_id = 'gdbt_{}_h_{}_{}_lr_{}_bs_{}_d_{}_sl_{}_{}_{}_{}_{}_{}/'.format(
