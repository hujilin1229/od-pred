...
def perm_data_od_pairs(x, indices):
    """
    ...
    # perm data with the multi-destinations
    final_perm = []
    for I_i in range(I):
        x_new_list = []
        for Oi in range(O):
        ...
        for d in range(Mnew):
            ...
            xnew = np.expand_dims(xnew, axis=-1)
            x_new_list.append(xnew)
        perm_orign = np.concatenate(x_new_list, axis=-1)
        perm_orign = np.expand_dims(perm_orign, axis=-1)
        final_perm.append(perm_orign)

    final_perm = np.concatenate(final_perm, axis=-1)

    return final_perm

def perm_data_rnn(x, indices):
    """
    Permute data matrix of od_pairs, i.e. exchange node ids,
    so that binary unions form the clustering tree.
    Note: the shape of od_pairs is (nb_sample, nb_edges, nb_intervals, nb_edges)
    ...
    for I_i in range(I):
        ...
        for i, j in enumerate(indices):
            ...
            # Or -infty ?
                ...
                xnew[:,i] = np.zeros(N)
        xnew = np.expand_dims(xnew, axis=1)
        final_perm.append(xnew)

    final_perm = np.concatenate(final_perm, axis=2)

    return final_perm

def perm_2d_data(x, indices):
    """
    Permute data matrix of od_pairs, i.e. exchange node ids,
    so that binary unions form the clustering tree.
